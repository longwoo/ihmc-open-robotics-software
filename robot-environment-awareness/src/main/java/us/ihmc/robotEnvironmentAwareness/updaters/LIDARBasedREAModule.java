...
public class LIDARBasedREAModule
{
   ...
   private final REAOcTreeBuffer lidar2BufferUpdater;
   ...
   private LIDARBasedREAModule(Messager reaMessager, File configurationFile) throws IOException
   {
      ...
      lidar2BufferUpdater = new REAOcTreeBuffer(mainOctree.getResolution(), reaMessager, REAModuleAPI.Lidar2BufferEnable, true,
                                                REAModuleAPI.Lidar2BufferOcTreeCapacity, 10000, REAModuleAPI.Lidar2BufferMessageCapacity, 500,
                                                REAModuleAPI.RequestLidar2Buffer, REAModuleAPI.Lidar2BufferState);
      ...
      REAOcTreeBuffer[] bufferUpdaters = new REAOcTreeBuffer[] {lidarBufferUpdater, lidar2BufferUpdater, stereoVisionBufferUpdater};
      ...
      ROS2Tools.createCallbackSubscription(ros2Node, LidarScanMessage.class, "/ihmc/lidar2_scan", this::dispatchLidar2ScanMessage);
      ...
      reaMessager.registerTopicListener(REAModuleAPI.SaveBufferConfiguration, (content) -> lidar2BufferUpdater.saveConfiguration(filePropertyHelper));
   ...
   private void dispatchLidar2ScanMessage(Subscriber<LidarScanMessage> subscriber)
   {
      LidarScanMessage message = subscriber.takeNextData();
      moduleStateReporter.registerLidar2ScanMessage(message);
      lidar2BufferUpdater.handleLidarScanMessage(message);
   }
   ...
   private void loadConfigurationFile(FilePropertyHelper filePropertyHelper)
   {
      ...
      lidar2BufferUpdater.loadConfiguration(filePropertyHelper);
   ...
   private void mainUpdate()
   {
      ...
      try
      {
         if (clearOcTree.getAndSet(false))
         {
            ...
            lidar2BufferUpdater.clearBuffer();
   ...
   public void start() throws IOException
   {
      if (scheduled == null)
      {
         ...
         executorService.scheduleAtFixedRate(lidar2BufferUpdater.createBufferThread(), 0, BUFFER_THREAD_PERIOD_MILLISECONDS, TimeUnit.MILLISECONDS);
